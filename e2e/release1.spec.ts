...
test.describe("admin Release 1", () => {
  ...
  test("signs in and opens the operational workspaces", async ({ page }) => {
    ...
    await page.getByRole("link", { name: /HIGH PRIORITY REVIEW/ }).first().click();
    await expect(page.getByRole("heading", { name: /HIGH PRIORITY REVIEW/ })).toBeVisible();
    await expect(page.getByRole("heading", { name: "Resolution actions" })).toBeVisible();
    await page.getByRole("link", { name: "Price Checks" }).click();
