...
Last updated: 2026-09-27（公开渠道受限首轮已完成）

## 2026-09-27 当前顺序

生产已在同一精确镜像上对 78 个无账号公开来源完成受限首轮：52 条定期计划运行，
25 个未过门槛的来源暂停，另一个 Lincoln 来源仅供受限验收、无计划。失败后的自动停采
仍生效。Waikato 和 AUT 的两个已完成 Argus 结果已经通过原 Job 保存业务数据、复制
并核验四份证据、ACK 和确认远端 PURGED；两来源仍暂停。精确结果和回滚材料见
[`evidence/public-source-production-rollout-2026-09-27.md`](./evidence/public-source-production-rollout-2026-09-27.md)。

下一步观察 52 条计划各自的下一个自然周期：核对 Job 终态、请求预算、来源身份集合、
新增/变化/撤稿、持久证据、去重、时效和自动停采。`FAILED_JOBS_PRESENT` 当前包含
保留的首轮失败历史，不能当作队列积压；要按新失败和已知历史分别判断。25 个暂停
来源逐一修复解析、来源契约或覆盖不足后，再执行新的有界两遍验收，不批量恢复。
Eventfinda 首轮没有业务结果，Ticketmaster 为解析失败，RBNZ 汇率与 MOT 航空数据
仍不可用；它们不属于已开放计划。客户入口、账号型 OTA、内部按需、支付及邮件继续关闭。

以下 2026-09-26 及更早段落保留当时的决策和证据，不描述当前部署。

## 2026-09-26 当前生产采集顺序

SSH 已恢复。生产 Worker/API/Scheduler 健康、队列为空；2026-09-26 UTC 的 GeoNet 和
ChristchurchNZ 每日 Job 均成功。ChristchurchNZ 官方场次 ID 在第二日变化，导致相同
来源活动和开场时间增加了重复来源场次；规范化的业务场次仍指向同一对象。已备份并
核验生产数据库，仅暂停 `first-christchurchnz-daily`，其他四条计划继续受限运行。
正式证据和精确备份位置见 [`traceability.md`](./traceability.md) 顶部。

本地修复已通过独立数据库的完整 `pnpm verify`，但尚未进入生产。下一步先确认
Argus 加固候选的生产发布门槛，发布精确镜像并核对回滚；随后只对 ChristchurchNZ
执行一次有限额验收，核对来源/规范场次 ID 集合、证据和请求预算。只有新增重复
为零、健康和失败停采仍有效时才恢复其每日计划。原有重复来源行的修复需单独
验证关联和历史保留约束。另三项每周计划仍待 2026-10-02 自然周期。

扩大到仓库其余渠道前，需要逐项确认生产来源配置、公开/账号边界、访问预算、
来源契约、持久化与去重、告警和回滚。当前生产 Scheduler 只接受五条精确计划；
通用开发 seed 不适用于生产。不得将尚未验证的渠道一次性标记为健康并启用。
...
| 门槛 | 需要记录的生产证据 | 当前边界 |
| --- | --- | --- |
| 五渠道第二周期 | 不同 UTC 日的五条精确计划、实际 Job ID/终态/尝试次数、各来源请求量与预算、来源业务 ID 前后集合、重复计数、Freshness、队列/告警及失败后停采 | 两条每日 Job 已完成；ChristchurchNZ 身份去重失败并暂停，三条每周 Job 待 2026-10-02 |
