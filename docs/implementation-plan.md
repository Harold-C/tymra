...
| P0 | 完成当前工作树质量门槛 | 专用测试库上的 `pnpm verify` 和完整 `pnpm test:e2e` 通过 | 当前代码已通过 lint、类型、137 个根测试（5 个外部 provider fixture 跳过）、76 个 Worker 测试、67 个隔离数据库/API/Worker 集成测试和生产构建；桌面/移动各 9 个 E2E 为此前已通过证据，本轮无 UI 改动未重跑 |
...
| P1 | 全国主要市场公开信号 | 15 个主要住宿市场具备全国发现、当地官方活动、住宿需求和扰动层 | Tymra 可直连的 14 个市场已实现；ADP、TVF、MRTE、IVS、Stats NZ、DOC 关闭、Interislander 提醒、三大滑雪季窗口与 Queenstown 月度客流已接入并通过有界实采/隔离落库；Dunedin 等待 Argus 连接器和跨服务两轮验收 |
...
- 全国主要市场当前实现口径与 2026-08-06 有界实采见
  [`collection/nz-market-public-signal-coverage.md`](./collection/nz-market-public-signal-coverage.md)。14 个直连市场不等于已证明多日稳定；Dunedin 仍等待 Argus 实现。
