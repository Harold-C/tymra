...
| Production build and runtime | Worker entrypoints built and Next.js generated 112 pages; Web image `sha256:bda56bdf4621c275aa0c5f245fbcdc608ae21a6e8d3963afb6522e8f3ca00c84` rebuilt, Compose Web/API are healthy and Worker is running; only the known optional LinkeDOM `canvas` warning appeared | verified |
...
| Payment/challenge/monitoring | Stripe Sandbox readiness 与真实浏览器生命周期通过；managed challenge invalid-token readiness、隐私安全 `/worker/alerts` 和单来源 2×2 canary 门槛已实现。Stripe live、生产 challenge/provider、dashboard、通知路由、容量和 SLA 尚未建立 | test_mode_verified_production_not_verified |
...
| `MEM-UNIT-001` | Pricing-unit list/detail GET, confirmed-owned-Price-Check POST, activation, deactivation, reactivation and downgrade selection | API integration now verifies list/detail/add, stable Property identity, unit limit and transactional cancellation; dedicated browser add/detail flow remains to be rerun | `implemented_not_verified` |
...
| `MEM-BILL-001` | Stripe Checkout/Portal and persisted upgrade/downgrade/cancel/resume/grace reconciliation | Fake-Stripe and isolated PostgreSQL lifecycle pass; 2026-08-12 real Sandbox browser acceptance covered Checkout, prorated upgrade, scheduled downgrade, schedule-aware cancel/resume and Portal with all 37 recent webhook records processed and zero errors. Stripe live remains gated off | `test_mode_verified_gated_off` |
