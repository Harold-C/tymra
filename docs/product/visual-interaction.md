...
登录页是低干扰单列卡片，只含邮箱、密码、必要披露和主按钮。凭证错误使用同一中性状态；不得用不同图标、颜色或文案透露账户是否存在。验证链接过期页提供重新发起对应验证流程和返回首页动作。

OTA URL、付款信息、营销预勾选和“必须先检查房源”的内容不得出现。密码字段必须使用正确的自动填充语义并允许密码管理器；Sign out 是明确文本动作，不只使用无标签图标。

## UI-MEMBER-3 账户概览

页面首屏按“当前状态 → 可用额度 → 定价单位 → 下一动作”排序。方案名称与价格不能成为唯一主角；会员首页必须帮助客户决定下一步。

关键指标最多使用可换行的紧凑网格：主动检查、有效单位、逐日价格检查范围、监测范围。日期范围显示实际起止日期，并说明 New Zealand time。付款逾期、取消待生效、方案待变更和 Launch Gate 使用不同的文字、图标和非颜色线索。
