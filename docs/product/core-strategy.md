...
技术层采用“数据新鲜度感知的缓存命中 \+ 全国持续采集 \+ 按需实时刷新 \+ 不可变历史存储”。正式报告必须固化一个时间一致、证据可追踪的 MarketSnapshot，并同时保留日期级 DateSnapshot、竞品集合版本、来源权利状态和分析版本；不同查询条件、不同 Sellable Unit 或相差过大的采集时间不得直接混合。  
...
* 定义 Market Reference／Review Range、Blocking Quality Gates、报告级与日期级置信度、证据、防滥用、来源权利和失败降级原则。  
...
9. 任何更多结果或操作，包括完整日期详情、历史、重新分析、竞品管理、全国深度数据、多房源、导出、持续监控和 PMS 连接，都必须注册正式账户；账户优先采用无密码 Magic Link。
...
Tymra Worker 的目标是在首期不依赖单一官方 OTA API、且不要求经营者先连接经营系统的情况下，结合主流 OTA 适配器、获批公共数据、许可数据和用户提供的信息开展全国市场研究。它必须确认 OTA Listing、Property 和 Sellable Unit，建立合理竞品组，查询标准化入住条件下的公开报价，识别全国及局部需求驱动因素，判断数据质量，然后给出可解释的市场参考和行动建议。  
...
* 任何价格都必须绑定 OTA Listing、Sellable Unit、完整 Stay Query、Collection Profile、采集时间、来源权利和证据。  
* 公开 OTA 报价是挂牌报价，不是实际成交 ADR。  
* 公开页面显示不可售不等于已经入住，也可能是关房、最少入住限制、渠道库存未开放或技术错误。  
* 历史观测采用只追加方式保存，不覆盖旧记录。  
* 匿名预览可以使用新鲜缓存、全国面板和有限实时查询，但仍不得展示无法验证的数值；邮箱正式结果必须通过更严格的新鲜度、覆盖率、可比性、来源权利和时间一致性质量门。  
...
* Listing 保存平台、外部 ID、OTA URL、平台房型名、首次发现时间、最后确认时间、在线状态、来源权利、与 Unit 的匹配置信度及身份合并历史。
...
* 保存采集时间、来源更新时间、页面 URL、查询参数、字段级证据、内容哈希、采集器与解析器版本、质量标记和权利状态；原始 HTML、网络响应和截图默认仅保存 72 小时，解析故障或审计需要时最多延长至 7 天。
...
* Eventfinda、Ticketmaster 和其他获批结构化来源用于建立全国活动与场地基础数据。  
...
* 开发环境必须实现统一 Data Source Adapter 层，并支持 Booking.com、Airbnb、Expedia、Hotels.com、Agoda、Trip.com、Google Hotels 及后续 Harold 批准的主流 OTA 或元搜索来源。Crawlee \+ Playwright 可作为首选执行基础，但具体工具选型由技术 ADR 管理。  
* 每个 OTA 适配器至少实现 identifyProperty、resolveListing、listUnits、fetchRates、fetchAvailability、fetchPolicies、healthCheck 和 rightsMetadata，并统一使用幂等、证据、并发限制、来源熔断和配置化任务协议。  
* 开发环境同时必须支持 LINZ、MBIE、Stats NZ、公众假期、学校假期、Eventfinda、Ticketmaster、场馆与 Council 日历、MetService、NZTA、GeoNet、机场、港口、邮轮、汇率和其他批准公共来源的适配器。Stagehand 仅可辅助页面探索和结构变化检测，不直接决定正式数值。  
...
6. 邮箱正式分析并行刷新目标 Listing 与 Unit、版本化 CORE 竞品、全国事件、天气、交通、需求和异常信号；只有获批生产来源的数据可以进入正式结果。  
...
* 对象存储：只在来源权利允许时短期保存原始响应、HTML、截图和网络证据；默认 72 小时，解析故障或审计需要时最多 7 天，随后自动删除。长期保留获准的标准化 Observation、字段级证据、哈希和来源追踪。  
...
* source\_url、evidence\_ref、collector\_version、parser\_version、quality\_flags、legal\_rights\_status、operational\_status
...
正式报告必须先通过 Blocking Quality Gates，再同时给出 Report-level Confidence 和 Date-level Confidence。阻断项至少包括：TARGET\_RATE\_MISSING、UNIT\_UNCONFIRMED、FEES\_UNKNOWN、COMPARABILITY\_FAILURE、SOURCE\_RIGHTS\_BLOCKED、SOURCE\_UNAVAILABLE、SEVERE\_CONFLICT、FRESHNESS\_EXPIRED、SNAPSHOT\_INCOHERENT 和 COMPETITOR\_COUNT\_BELOW\_3。置信度至少由以下部分组成：
...
竞品目标为 8–20 个有效唯一 CORE Unit；至少 8 个且核心数据在 24 小时内可形成 High Confidence，5–7 个可形成 Medium，3–4 个只能输出 Low Confidence 方向性结果，少于 3 个必须返回 Insufficient Data。一个 APPROVED 价格来源可形成正式结果，第二来源用于交叉验证和提升置信度。采集失败、覆盖不足或阻断项命中时必须降低置信度或拒绝生成精确判断，任何情况下不得回退到随机 Demo 数据。
...
* 通过质量门后固化报告级 MarketSnapshot、日期级 DateSnapshot、竞品集合版本、来源权利状态和分析版本。  
...
Airbnb 和 Booking.com 当前条款对自动化访问、机器人、抓取器及某些浏览器自动化助手存在明确限制。开源工具、Apify Actor、代理或浏览器模拟不能消除这一来源权利问题。

* 每个来源建立 SourceRegistry，分别记录 internal\_approval\_status、legal\_rights\_status 和 operational\_status，以及访问方式、许可依据、允许用途、展示权限、衍生分析权限、保留策略、并发预算、负责人和最近复核时间。Harold 是当前统一的内部最终批准人。  
* 来源可以分别进入 RESEARCH、POC、PILOT 或 PRODUCTION。只有 Harold 内部批准、legal\_rights\_status 允许、技术稳定性通过且 operational\_status 健康的来源，才能进入正式自动发布。  
* 无法确认权利的来源标记 RIGHTS\_REVIEW，明确受限或禁止的来源标记 RIGHTS\_BLOCKED。Harold 可以批准研究或 POC，但内部批准不能把明确禁止商业使用的来源转化为合法生产来源；未获生产准入的数据不得进入正式报告。  
* 优先使用官方开放数据、明确 API、许可数据、合作数据和可证明允许商业使用的来源。  
* 在来源允许、技术可行和成本合理的范围内尽量采集所有可能影响身份、价格、可售、竞品、需求和解释的字段。原始页面、截图和网络响应默认保存 72 小时，故障或审计需要时最多 7 天，并在到期后自动删除；Cookie、Session、Token 和个性化标识立即清除。获准的标准化事实、字段级证据、内容哈希和衍生指标可以长期保留，并持续追踪来源。

Harold 当前统一批准来源的开发、研究、POC、Pilot 和内部生产启用状态，并可暂停或撤回来源。涉及明确条款解释、重大不确定性或规模化商业使用时仍应取得独立法律审查；本文件记录产品和技术边界，不构成法律意见。

# 18\. 开发、演示与生产数据

* Development 环境必须能够运行主流 OTA 与全国公共来源适配器的研究、fixture、sandbox 或获批测试模式；Development Demo Data 只用于验证流程行为，不是真实市场数据，也不代表生产来源准入。  
...
* 确认主流 OTA 与公共来源适配器契约、默认 Query Plan、SourceRegistry、Harold 内部批准流程、生产权利门和对外报告声明。  
...
* POC 和开发适配器只用于技术、字段与数据质量验证，不自动等同于生产准入；正式结果只使用 Harold 批准且法律权利允许的生产来源。
...
* 实现邮箱验证与 RESULT\_READY 安全结果链接；链接只授权查看当前有限报告，不自动建立完整账户。用户请求完整明细、历史、重新分析、竞品管理、全国深度数据或持续监控时，再通过无密码 Magic Link 注册正式账户。  
...
* Harold 内部批准记录、legal\_rights\_status、生产白名单、字段级证据、72 小时／7 天原始素材删除、标准化数据来源追踪和来源停用审计通过率。
...
* 不因为市场上存在 scraper 就忽略来源权利。  
* 不为一次分析发送注册、验证、登录和报告四封重复事务邮件；正常成功路径只发送 RESULT\_READY，注册账户仅在用户请求更多能力时独立发生。

# 22\. 已确认默认值与剩余待验证事项

* 已确认：开发环境支持 Booking.com、Airbnb、Expedia、Hotels.com、Agoda、Trip.com、Google Hotels 及全国推荐公共来源；生产白名单由 Harold 根据内部批准、法律权利、技术稳定性和运行健康逐项启用。  
...
* 已确认：正式报告先通过 Blocking Quality Gates，并同时输出 Report-level 与 Date-level Confidence；一个 APPROVED 价格来源可形成正式结果，第二来源用于交叉验证和提高置信度。匿名只展示有限真实预览，邮箱链接只展示有限只读正式报告，更多结果或操作必须注册账户。  
* 已确认：Harold 是当前内部最终批准人；SourceRegistry 分离 internal\_approval\_status、legal\_rights\_status 与 operational\_status。生产白名单必须同时满足 Harold 批准、法律权利允许、技术稳定和运行健康；原始素材默认 72 小时、必要时最多 7 天后删除。  
...
* 可复用的开源浏览器自动化、采集框架或市场 scraper 只作为技术参考和受控 POC 候选，不自动提供生产数据权利，也不应成为单一供应商依赖。  
* 每个生产来源必须同时记录内部批准、法律权利、运行状态、允许用途、证据保留策略、健康指标、成本与停用开关；技术可访问不等于可以商业使用。  
