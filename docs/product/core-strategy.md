...
- `LISTING_PRICING`：确认受支持的目标 OTA Listing，返回所有有效目标房源公开价格，并结合周边竞品及公共市场信号判断是否可给出调价建议。
- `LOCATION_BENCHMARK`：确认真实的新西兰地址，无需目标 OTA Listing；返回所有符合入住条件的附近公开住宿价格，并明确标记为周边行情基准，绝不伪装成该地址自己的价格。
- 两种模式都必须绑定稳定的物理 `Property` 身份、明确入住条件、NZD 和 `Pacific/Auckland` 日期。同一实体住宿的地址与 OTA 链接共用一个房源额度。
- 任一模式只要取得至少一个有效且可归属的公开 OTA 价格，就必须成功返回该价格；竞品数量或市场信号不足只影响推荐，不得把已观察价格降级为 `PARTIAL` 或 `INSUFFICIENT_DATA`。
...
Tymra 的核心是以住宿定价为起点建设“新西兰住宿市场情报基础设施”：持续收集并关联与住宿需求、供给、价格、可售和异常变化有关的合法数据，将其转化为长期连续、可追溯、可比较的市场资产。Tymra 当前不是覆盖所有行业、所有主题的通用新西兰数据中心；是否扩展到住宿以外的领域，必须由已有数据优势、明确客户问题、合法用途和独立商业验证共同决定。

会员端是第一个商业产品出口，与数据后台一起部署但初期隐藏公开入口，面向住宿经营者提供以下能力：

1. 接收真实新西兰地址或受支持 OTA Listing URL，并确认稳定的物理 Property 身份；
2. 映射目标 Listing、Sellable Unit 和有比较价值的周边住宿；
3. 查询适用于指定条件的公开 OTA 价格和可售状态；
4. 将实时价格与持续积累的公共市场信号、历史变化、Freshness 和 Confidence 结合；
5. 输出可解释的逐日价格位置、调价建议、限制和持续监测结果。

同一数据核心未来可以支持酒店集团组合分析、区域旅游市场、住宿投资研究、活动影响评估以及受控 API／数据服务等不同产品端。它们是共享数据基础设施上的独立产品，不是当前会员端自动获得的隐藏功能，也不因数据已经存在就自动取得上线、营销或商业使用资格。

核心数据资产至少由以下七部分构成：

* 统一 SourceRegistry 及来源运行、能力和保留边界；
* 来源发布时间／生效时间、采集时间、业务适用日期和入库时间；
* 短期原始证据、长期标准化事实和分析衍生结果的分层；
* 字段、记录、快照和结果级 Confidence／Freshness；
* 真实地址、Property、Sellable Unit 与多 OTA Listing 的版本化身份映射；
* Listing、房型、公开价格、可售、政策及状态的追加变化历史；
* 从结论反向追溯到快照、标准化记录、Collection Run、来源和证据哈希的 Data Lineage。

因此，页面、会员方案和单次比价流程是商业化界面；长期连续的数据覆盖、稳定身份、历史深度、质量校准和来源链路才是难以复制的核心资产。
...
* 定义从全国计划采集或内部按需输入到稳定身份、缓存命中、按需刷新、标准化事实和可追溯分析的统一流程。客户流程由独立产品契约控制。
...
本文件是 Tymra 的长期战略与目标架构文件，负责产品使命、全国数据壁垒、目标能力和不可违反原则。当前开发版本的具体范围与对外承诺由《需求说明》控制；全国数据核心的可执行契约由《data-core.md》控制；领域状态与默认值由《业务规则》控制；客户路由与用户流程由《页面结构》和《customer-funnel.md》控制；视觉实现由《视觉交互》控制。技术工具、数据库迁移、接口和部署方式进入技术方案或 ADR；算法权重、内部阈值和特征组合进入受限决策文档。发生冲突时按决策领域服从对应权威文件。
...
* `LISTING_PRICING` 在正式分析前必须解析、匹配并确认至少一个有效目标 OTA Listing URL，并绑定到具体 Property 和 Sellable Unit。
* `LOCATION_BENCHMARK` 可以用确认后的真实新西兰地址作为空间锚点；附近 OTA Listing 是周边价格证据，不是该地址的目标房源身份或自有价格。
* 存在多个 Property、Sellable Unit 或 Listing 候选时必须要求用户确认；无法自动确认时进入 NEEDS\_CONFIRMATION，并允许用户补充 OTA 链接。  
* Listing 模式无法绑定有效目标 OTA Listing 时不得生成目标房源价格分析；地址模式无法可靠确认地址或没有有效附近公开价格时必须返回明确受限状态，不得猜测价格。

## 2.2 默认查询与 Sellable Unit 确认

* 匿名首次体验不要求用户选择日期、入住人数或房型，以减少首次使用摩擦。  
* 统一默认 Stay Query 为 2 名成人、0 名儿童、1 个住宿单位、1 晚、NZD 和 `Pacific/Auckland`；页面中的明确查询条件可以作为候选条件保存，但必须标准化后使用。
* 匿名／一次性正式检查覆盖未来 30 天并最多输出 5 个重点日期；登录会员的精确逐日价格范围按 Free 14、Host 30、Pro 90、Portfolio 180 天执行，不能把 30 天固定值用于所有方案。
...
1. 调度器或内部运营者选择全国计划采集、地址周边行情或目标 OTA Listing 采集。
2. 系统确认区域、Property／空间锚点、Sellable Unit、Listing、查询条件和所需来源 capability。
3. Worker 复用精确且新鲜的事实，并对陈旧、缺失、近期、高影响或明确指定的范围执行有界刷新。
4. 后台展示真实 Collection Run、来源健康、覆盖、Freshness、Confidence、异常和 lineage；不得用模拟进度或 fixture 掩盖失败。
5. 标准化事实和衍生结果追加保存；短期 Argus 证据经核验和 ACK 后按合同 purge。
6. 客户身份、会员、公开 Price Check 和结果交付随生产版本部署；初期只从首页、公共导航和营销 CTA 隐藏入口。

## 2.4 客户访问与通知

会员使用邮箱用户名和密码登录；Magic Link 仅用于匿名 Price Check 的一次性验证与会话建立，不是会员登录入口。正常流程使用 `VERIFY_AND_SIGN_IN`，只有正式结果未在通知宽限期内完成已认证页面交付时才发送 `RESULT_READY` 或对应终态邮件。完整状态、幂等、同意和通知规则只由《customer-funnel.md》和《membership-plans.md》维护，本文不再复制另一套流程。
...
* 全国房源与 Listing 目录、全国代表性价格面板、全国事件和异常信号采集机制属于当前统一生产范围；全国价格矩阵不追求所有房源和日期组合全量采集。
...
* 内部和客户结果可以使用新鲜缓存、全国面板和有界实时查询，但不得展示无法验证的数值；客户结果必须通过完整质量门槛。
...
* 保存来源发布时间／生效时间、采集时间、业务适用日期、入库时间、页面 URL、查询参数、字段级证据引用、内容哈希、采集器与解析器版本和质量标记。Argus Job 在结果与证据哈希核验并 ACK 后可以立即 purge；需要延长诊断材料时必须在 ACK 前走隔离流程，不以固定 72 小时／7 天覆盖 Argus 合同。
...
* 开发环境必须实现 capability-based Data Source Adapter 层。当前 OTA 固定为 Booking.com、Airbnb、Expedia、Bookabach、Agoda 和 Trip.com；其他 OTA 不进入当前合同、执行或验收范围。浏览器执行由独立 Argus 服务负责，具体边界由技术 ADR 管理。
* OTA 来源按真实能力注册 discoverListings、resolveListing、collectRates、collectAvailability、collectPolicies 和 healthCheck；事件、大学日期、交通、天气或导入来源只注册适用能力。编排器在执行前检查 capability、启用和健康状态，不要求非 OTA 来源提供空的房源／房价方法。
...
正确实现不是每次全部实时搜索，也不是直接读取历史缓存，而是“全国持续采集 \+ 新鲜度感知的缓存命中 \+ 按需实时刷新 \+ 历史追加存储”。全国底座先运行，计划任务或内部请求只补齐真正缺失、过期或需要更窄时间窗口的数据。

1. 收到 ScheduledCollectionRequest 或 InternalCollectionRequest，生成请求指纹、幂等键和任务范围；相同 Listing、Unit 和 Query Plan 优先合并任务。
2. 解析并确认分析模式、Property、地理市场、标准 Query Plan 和 Collection Profile；Listing 模式还必须确认目标 OTA Listing 与 Sellable Unit，地址模式必须确认地址空间锚点且不得伪造目标 Unit。
3. 从存储中查找完全匹配的 RateObservation、EventObservation 和需求信号。  
4. Freshness Evaluator 将结果分类为新鲜、部分、过期、缺失或冲突。  
5. 内部查询优先使用新鲜精确命中和全国面板；部分、过期或冲突数据明确标识并按策略触发受限刷新。
6. 需要分析时并行刷新目标 Listing／空间锚点、版本化 CORE 竞品、全国事件、天气、交通、需求和异常信号；只有具备所需 capability、已启用且运行健康的来源数据可以进入结果。
...
9. PriceAnalysis 保存市场参考区间、重点日期、风险、置信度和证据；需要固化时创建不可变 Result Version。重新分析或人工修正必须创建新版本，不覆盖历史结果；客户通知按《customer-funnel.md》执行。
...
* 对象存储：按具体执行合同短期保存原始响应、HTML、截图和网络证据。Argus 在结果和证据哈希核验并正确 ACK 后可以立即 purge；需要延长诊断证据时必须在 ACK 前隔离。长期保留标准化 Observation、必要的证据引用／哈希和来源追踪。
...
* analysis\_mode、analysis\_request\_id、target\_property\_id、target\_sellable\_unit\_id、target\_listing\_id、spatial\_anchor、query\_plan\_id、query\_plan\_version、competitor\_set\_version、as\_of、market\_scope。`LISTING_PRICING` 要求真实 target\_listing\_id；`LOCATION_BENCHMARK` 要求稳定 Property／spatial anchor，并允许 target\_listing\_id 为空。
...
竞品目标为 8–20 个有效唯一 CORE Unit；至少 8 个且核心数据通过 Freshness 策略可形成 High Confidence，5–7 个可形成 Medium，3–4 个只能输出 Low Confidence 方向性推荐，少于 3 个不得生成竞品区间或调价建议。一个有效且可归属的公开价格即可形成成功价格结果，第二来源只用于交叉验证和提升证据质量。采集失败、覆盖不足或阻断项命中时必须降低推荐置信度或拒绝生成精确建议，但不得隐藏已验证价格，也不得回退到随机 Demo 数据。

# 15\. 客户产品（当前 `DEPLOYED_HIDDEN`）

客户产品与采集后台进入同一生产部署，初期只隐藏首页、公共导航和营销 CTA 入口。认证、通知和会员权益如与本章文字冲突，以《customer-funnel.md》和《membership-plans.md》为准。

客户端使用 `LISTING_PRICING` 与 `LOCATION_BENCHMARK` 两种明确模式。至少一个有效公开价格即可使价格结果成功；竞品或市场信号不足只限制推荐。会员采用邮箱用户名和密码登录；匿名 Price Check 使用一次性 `VERIFY_AND_SIGN_IN` 建立已验证会话，终态邮件按在页交付状态有条件发送。Free、Host、Pro、Portfolio 的日期范围、额度、监测、保留和收费只引用《membership-plans.md》，不得在本战略文件复制固定值。

# 16\. 防滥用、成本与可靠性

客户相关额度与防滥用条款属于当前部署契约；内部采集同时执行幂等、并发、来源预算、熔断和异常枚举保护。
...
* 正式结果使用已认证客户会话和服务端资源归属检查，不使用持久 bearer 结果链接。一次性验证 token 只保存 hash、15 分钟有效、单次消费并从 URL 移除；token、明文凭证和来源账户秘密不得进入日志、分析事件、错误追踪或第三方请求。

# 17\. 来源运行安全

* 每个来源建立 SourceRegistry，记录稳定 Source Key、数据域、capability、启用状态、生命周期、运行健康、访问方式、保留策略、并发预算、负责人和最近检查时间。
...
* 优先使用稳定、可追踪的官方公开数据、API 和合作数据。
* 在技术可行和成本合理的范围内采集影响身份、价格、可售、竞品、需求和解释的字段。原始页面、截图和网络响应按具体来源与执行合同清理；Argus 证据在正确 ACK 后可以立即 purge。Cookie、Session、Token 和个性化标识不得进入长期存储。标准化事实、必要的证据引用／哈希和衍生指标按数据类别策略保留，并持续追踪来源。
...
以下阶段只表示同一开发版本内部的实现和验收顺序，不表示分批上线。首次生产部署必须同时包含各阶段形成的后台、客户会员与支付能力；客户端初期仅隐藏公开发现入口。
...
* 在统一生产候选冻结前，全国采集机制持续运行并验证调度、重试、熔断、来源健康、追加历史、覆盖率、成本和异常告警稳定；客户端能力同时完成生产验收，公开入口是否显示由独立展示配置控制。
* POC 和开发适配器只用于技术、字段与数据质量验证，不自动等同于生产启用；正式结果只使用已启用且运行健康的生产来源。

## 阶段 2：全国 Market Explorer 与历史底座

* 开发顺序可先完成内部运营后台的全国 Market Explorer，展示各 Region、Territorial Authority、微市场和住宿类别的价格区间、公开可售变化、事件、异常、覆盖率和数据时间；该顺序不构成独立后台发布。
...
## 阶段 4：客户端同版本部署（当前 `DEPLOYED_HIDDEN`）

* 实现强制 OTA Listing 解析、匿名真实预览、反滥用、邮箱收集、正式刷新、MarketSnapshot／DateSnapshot 和已认证客户结果。
* 会员使用邮箱和密码登录；匿名 Price Check 解锁使用一次性 `VERIFY_AND_SIGN_IN` 建立已验证会话，`RESULT_READY` 仅在认证页面未完成在页交付时有条件发送。
...
* 从同版本部署开始跟踪 Price Check 转化率、缓存复用率、CAPTCHA 触发率、防滥用误伤率和异常批量请求阻断率；入口隐藏期间将公开转化指标标记为不可评价。
* 从同版本部署开始跟踪 `VERIFY_AND_SIGN_IN`、条件终态通知、认证结果查看、重复邮件和无效 token 访问率。
...
* 每个全国市场每日运行、内部按需采集和每个有效观察的成本，以及缓存复用率和来源成本占比。
* 生产来源 capability／启用／健康状态、Argus ACK／purge、标准化数据 lineage 和来源停用审计通过率。

# 21\. 当前明确不做的事情

* 自然地址可以生成明确标记的周边行情基准，但没有目标 OTA Listing 时不生成该地址的自有价格、目标房源调价结论或精确收益结论。
...
* 客户端不得为一次分析发送注册、验证、登录和报告四封重复事务邮件，通知规则以《customer-funnel.md》为准。

# 22\. 已确认默认值与剩余待验证事项

* 已确认：当前 OTA 为 Booking.com、Airbnb、Expedia、Bookabach、Agoda、Trip.com，并支持全国公共来源；测试和生产来源根据启用状态、技术稳定性和运行健康逐项启用。其他 OTA 不进入合同、发现和健康门槛。开发环境硬性禁止自动调度。
* 已确认：用户正式检查默认 2 名成人、0 名儿童、1 个住宿单位、1 晚、NZD 和 `Pacific/Auckland`；匿名流程默认未来 30 天，会员流程使用方案对应的精确逐日范围；全国内部面板使用 D+7、D+14、D+30、D+60、D+90，并补充工作日、周末、假期和批准活动日期。
* 已确认：初始全国代表性面板为 1,000–1,500 个唯一 Sellable Unit，约 80% Anchor、20% Rotating；正式竞品目标 8–20 个，5 个为推荐最低可靠基准，3–4 个只允许 Low Confidence 推荐，少于 3 个不生成竞品区间或调价建议，但仍返回任何有效公开价格。
* 已确认：核心目标价和主要竞品默认不得超过 24 小时；D+0 至 D+7 或重大活动日期的目标与 CORE 竞品最大采集偏差为 2 小时，D+8 至 D+30 为 6 小时；具体新鲜度策略保持服务端可配置并版本化。  
* 已确认：价格事实与调价推荐使用独立状态；一个有效且可归属的公开价格形成成功价格结果，第二来源用于交叉验证和提高证据质量。客户结果随统一生产版本部署，初期只隐藏公开发现入口。
* 已确认：SourceRegistry 记录来源 capability、启用状态、生命周期与运行状态；原始素材按具体执行合同清理，Argus 证据在正确 ACK 后可以立即 purge。
