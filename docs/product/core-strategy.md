...
文档状态：Active — Tymra Core Strategy Baseline v1.1｜Owner / 最终批准人：Harold｜最后更新：2026 年 8 月 11 日

## Post-Release 地址分析修订（2026-08-11）

本节覆盖本文中“必须先绑定目标 OTA Listing 才能生成任何正式结果”的 Release 1 历史限制。当前分析入口及价格返回规则以《membership-plans.md》为权威来源：
...
* Release 1 匿名/一次性正式检查覆盖未来 30 天并最多输出 5 个重点日期；登录会员的精确逐日价格范围按 Free 14、Host 30、Pro 90、Portfolio 180 天执行，不能把 30 天固定值用于所有方案。
...
1. 用户选择具体 OTA 房源定价或地址周边行情基准，并输入已支持的 OTA Listing URL 或新西兰地址。
2. 系统识别 Property、Sellable Unit、市场和候选 Listing；Listing 模式确认目标 Listing，地址模式确认空间锚点和附近公开价格证据。
...
* Property name 和自然地址保留为发现入口，因为它们有利于降低首次使用门槛。Listing 模式必须解析并确认有效目标 OTA Listing、具体 Property 与 Sellable Unit；地址模式可以在确认真实 Property 空间锚点后返回附近公开价格基准，但不得把附近房源当作目标房源或形成无证据的自有价格结论。
