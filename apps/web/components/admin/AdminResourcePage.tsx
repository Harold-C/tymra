...
function SettingsPage({ locale, title, description }: { locale: AdminLocale; title: string; description: string }) {
  ...
  const settings = [
    ["Provider mode", environment.PROVIDER_MODE], ["Public collection mode", environment.PUBLIC_COLLECTION_MODE], ["Scheduler", yesNo(environment.SCHEDULER_ENABLED, locale)], ["Default market", environment.DEFAULT_MARKET], ["Auto publish", yesNo(environment.AUTO_PUBLISH_ENABLED, locale)], ["Accept new checks", yesNo(environment.ACCEPT_NEW_CHECKS, locale)], ["Email provider", environment.EMAIL_PROVIDER], ["Result link lifetime", locale === "zh" ? `${environment.RESULT_LINK_TTL_DAYS} 天` : `${environment.RESULT_LINK_TTL_DAYS} days`], ["Worker lease", locale === "zh" ? `${environment.WORKER_LEASE_SECONDS} 秒` : `${environment.WORKER_LEASE_SECONDS} seconds`], ["Application URL", environment.APP_BASE_URL],
  ];
  const settingsLabels: Record<string, string> = locale === "zh" ? { "Provider mode": "数据提供模式", "Public collection mode": "公开采集模式", Scheduler: "定时任务", "Default market": "默认市场", "Auto publish": "自动发布", "Accept new checks": "接受新检查", "Email provider": "邮件服务", "Result link lifetime": "结果链接有效期", "Worker lease": "Worker 租约", "Application URL": "应用地址" } : {};
