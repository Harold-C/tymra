...
function SettingsPage({ locale, title, description }: { locale: AdminLocale; title: string; description: string }) {
  ...
  const settings = [
    ["collection", "Provider mode", environment.PROVIDER_MODE], ["collection", "Public collection mode", environment.PUBLIC_COLLECTION_MODE], ["collection", "Scheduler", yesNo(environment.SCHEDULER_ENABLED, locale)], ["collection", "Internal on-demand", yesNo(environment.INTERNAL_ON_DEMAND_ENABLED, locale)], ["market", "Default market", environment.DEFAULT_MARKET], ["delivery", "Auto publish", yesNo(environment.AUTO_PUBLISH_ENABLED, locale)], ["delivery", "Accept new checks", yesNo(environment.ACCEPT_NEW_CHECKS, locale)], ["membership", "Customer funnel", yesNo(environment.CUSTOMER_FUNNEL_ENABLED, locale)], ["delivery", "Email provider", environment.EMAIL_PROVIDER], ["collection", "Worker lease", locale === "zh" ? `${environment.WORKER_LEASE_SECONDS} 秒` : `${environment.WORKER_LEASE_SECONDS} seconds`], ["platform", "Application URL", environment.APP_BASE_URL],
  ];
  const settingsLabels: Record<string, string> = locale === "zh" ? { "Provider mode": "数据提供模式", "Public collection mode": "公开采集模式", Scheduler: "定时任务", "Default market": "默认市场", "Auto publish": "自动发布", "Accept new checks": "接受新检查", "Customer funnel": "客户漏斗", "Email provider": "邮件服务", "Worker lease": "Worker 租约", "Application URL": "应用地址" } : {};
