...
export function MemberSignInForm({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!payload) throw new Error(copy("serviceError"));
      if (!response.ok || !payload.data?.returnTo) throw new Error(payload.error?.message ?? copy("error"));
      router.push(payload.data.returnTo);
      router.refresh();
    } catch (caught) {
      setError(caught instanceof TypeError ? copy("serviceError") : caught instanceof Error ? caught.message : copy("error"));
    } finally {
      setBusy(false);
    }
  }
  ...
  return <section className="member-signin-card" aria-labelledby="member-signin-heading">
    ...
    <form onSubmit={submit}>
      ...
      <input id="member-password" name="password" type="password" autoComplete="current-password" defaultValue={defaultPassword} minLength={12} maxLength={200} required />
