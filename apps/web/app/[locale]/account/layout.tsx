...
export default async function AccountLayout({ children, params }: { children: React.ReactNode; params: { locale: "en" | "zh" } }) {
  ...
  return <PublicShell locale={params.locale}><CustomerAccountNav locale={params.locale} plan={session.customerUser.membership?.plan ?? "FREE"} features={membershipFeatureLaunchAvailability()} />{children}</PublicShell>;
}
