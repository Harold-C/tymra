...
export async function POST(request: NextRequest) {
  try {
    ...
    return apiSuccess(await createMembershipCheckout(session.customerUserId, inputSchema.parse(await request.json()).plan));
  } catch (error) {
    if (error instanceof CustomerAuthenticationError) return apiError(401, "CUSTOMER_AUTH_REQUIRED", error.message);
    if (error instanceof BillingError) return apiError(error.code === "BILLING_DISABLED" || error.code === "PLAN_NOT_AVAILABLE" ? 409 : 422, error.code, error.message);
