...
export default async function MarketIntelligencePage({ searchParams }: { searchParams: { view?: string; scope?: string; q?: string; region?: string } }) {
  ...
  const result = view === "events" ? await loadEvents({ q, region, current: scope === "current", now, locale }) : await loadSignals({ q, region, current: scope === "current", now, locale });
  ...
  return <section className="admin-page market-intelligence-page">
    ...
    <form className="admin-filters" method="get"><input type="hidden" name="view" value={view} /><label>{text.scope}<select name="scope" defaultValue={scope}><option value="current">{view === "events" ? text.upcomingOnly : text.activeOnly}</option><option value="all">{text.allRecords}</option></select></label><label>{text.region}<select name="region" defaultValue={region}><option value="">{text.allRegions}</option>{regions.map((item) => item.region ? <option key={item.region}>{item.region}</option> : null)}</select></label><label>{text.search}<input name="q" defaultValue={q} placeholder={text.searchPlaceholder} /></label><button className="button button-secondary" type="submit">{text.apply}</button></form>
    <div className="data-explorer-result-meta"><strong>{view === "events" ? text.canonicalEvents : text.canonicalSignals}</strong><span>{text.count.replace("{count}", String(result.rows.length))}</span></div>
    <AdminTable columns={result.columns} rows={result.rows} emptyTitle={text.empty} emptyBody={text.emptyBody} />
  </section>;
}

async function loadEvents(input: { q: string; region: string; current: boolean; now: Date; locale: AdminLocale }) {
  const where: Prisma.EventOccurrenceWhereInput = { isDemo: false, ...(input.current ? { startsAt: { gte: input.now } } : {}), ...(input.region ? { venue: { region: input.region } } : {}), ...(input.q ? { OR: [{ canonicalEvent: { title: { contains: input.q, mode: "insensitive" } } }, { venue: { city: { contains: input.q, mode: "insensitive" } } }, { venue: { name: { contains: input.q, mode: "insensitive" } } }] } : {}) };
  const items = await prisma.eventOccurrence.findMany({ where, orderBy: { startsAt: input.current ? "asc" : "desc" }, take: 100, include: { canonicalEvent: true, venue: true, sourceLinks: { include: { sourceEventOccurrence: { include: { dataSource: { select: { name: true } } } } } } } });
  return { columns: [{ key: "event", label: input.locale === "zh" ? "事件" : "Event" }, { key: "location", label: input.locale === "zh" ? "地点" : "Location" }, { key: "starts", label: input.locale === "zh" ? "开始时间" : "Starts" }, { key: "status", label: input.locale === "zh" ? "状态" : "Status" }, { key: "impact", label: input.locale === "zh" ? "价格影响" : "Impact" }, { key: "sources", label: input.locale === "zh" ? "来源" : "Sources" }], rows: items.map((item) => ({ id: item.id, cells: { event: <><strong>{item.canonicalEvent.title}</strong><small>{item.canonicalEvent.category ?? "—"}</small></>, location: [item.venue?.name, item.venue?.city, item.venue?.region].filter(Boolean).join(" · ") || "—", starts: date(item.startsAt, input.locale), status: <StatusPill value={item.status} locale={input.locale} />, impact: <><StatusPill value={item.impactStatus} locale={input.locale} />{item.impactScore !== null ? <small>{Math.round(item.impactScore * 100)}%</small> : null}</>, sources: [...new Set(item.sourceLinks.map((link) => link.sourceEventOccurrence.dataSource.name))].join(", ") || "—" } })) };
}

async function loadSignals(input: { q: string; region: string; current: boolean; now: Date; locale: AdminLocale }) {
  const where: Prisma.MarketSignalWhereInput = { isDemo: false, ...(input.current ? { endsAt: { gte: input.now } } : {}), ...(input.region ? { region: input.region } : {}), ...(input.q ? { OR: [{ marketKey: { contains: input.q, mode: "insensitive" } }, { region: { contains: input.q, mode: "insensitive" } }, { status: { contains: input.q, mode: "insensitive" } }] } : {}) };
  const items = await prisma.marketSignal.findMany({ where, orderBy: { startsAt: input.current ? "asc" : "desc" }, take: 100, include: { dataSource: { select: { name: true } }, sourceLinks: { include: { sourceMarketSignal: { include: { dataSource: { select: { name: true } } } } } } } });
  return { columns: [{ key: "signal", label: input.locale === "zh" ? "信号" : "Signal" }, { key: "market", label: input.locale === "zh" ? "市场" : "Market" }, { key: "region", label: input.locale === "zh" ? "地区" : "Region" }, { key: "window", label: input.locale === "zh" ? "有效期" : "Window" }, { key: "status", label: input.locale === "zh" ? "状态" : "Status" }, { key: "sources", label: input.locale === "zh" ? "来源" : "Sources" }], rows: items.map((item) => ({ id: item.id, cells: { signal: formatAdminValue(input.locale, item.type), market: item.marketKey, region: item.region, window: <>{date(item.startsAt, input.locale)}<small>{date(item.endsAt, input.locale)}</small></>, status: <StatusPill value={item.status} locale={input.locale} />, sources: item.dataSource?.name ?? ([...new Set(item.sourceLinks.map((link) => link.sourceMarketSignal.dataSource.name))].join(", ") || "—") } })) };
}
