...
export default async function ProductionReadinessPage() {
  ...
  const [sources, enabledSchedules, failedJobs, argus] = await Promise.all([
    prisma.dataSource.findMany({ where: { providerType: "PUBLIC", isDemo: false }, orderBy: { key: "asc" }, select: { key: true, name: true, enabled: true, internalApprovalStatus: true, legalRightsStatus: true, operationalStatus: true, rightsAllowStorage: true, rightsAllowDerivedAnalysis: true } }),
  ...
  const external = failedJobs.filter((job) => /RIGHTS_BLOCKED|SOURCE_UNAVAILABLE|RATE_LIMITED|ACCESS_CHALLENGE/.test(job.lastErrorCode ?? "")).length;
  const retryable = failedJobs.filter((job) => job.status === "FAILED" && !/RIGHTS_BLOCKED|SOURCE_UNAVAILABLE|RATE_LIMITED|ACCESS_CHALLENGE/.test(job.lastErrorCode ?? "") && /TIMEOUT|NETWORK|LEASE|UNAVAILABLE/.test(job.lastErrorCode ?? "")).length;
  const sourceBlockers = candidates.flatMap((source) => {
    const reasons = [
      ...(source.internalApprovalStatus !== "APPROVED" || source.legalRightsStatus !== "ALLOWED" ? [t.approvalBlocked] : []),
      ...(source.operationalStatus !== "HEALTHY" ? [t.healthBlocked] : []),
      ...(!source.rightsAllowStorage || !source.rightsAllowDerivedAnalysis ? [t.rightsBlocked] : []),
    ];
    return reasons.length ? [`${source.key}: ${reasons.join(" · ")}`] : [];
  });
  const blockers = [
    ...(environment.SCHEDULER_ENABLED ? [t.schedulerRuntimeBlocked] : []),
    ...(enabledSchedules ? [t.schedulesBlocked.replace("{count}", String(enabledSchedules))] : []),
    ...(!argus.healthy || !argus.ready ? [t.argusBlocked] : []),
    ...sourceBlockers,
  ];
  const cards = [
    { label: t.rights, value: `${candidates.filter((source) => source.rightsAllowStorage && source.rightsAllowDerivedAnalysis).length} / ${candidates.length}`, ok: candidates.every((source) => source.rightsAllowStorage && source.rightsAllowDerivedAnalysis), detail: t.rightsDetail },
    { label: t.approval, value: `${candidates.filter((source) => source.internalApprovalStatus === "APPROVED" && source.legalRightsStatus === "ALLOWED").length} / ${candidates.length}`, ok: candidates.every((source) => source.internalApprovalStatus === "APPROVED" && source.legalRightsStatus === "ALLOWED"), detail: t.approvalDetail },
    ...
    { label: t.schedules, value: String(enabledSchedules), ok: enabledSchedules === 0 && !environment.SCHEDULER_ENABLED, detail: environment.SCHEDULER_ENABLED ? t.schedulerOn : t.schedulerOff },
...
const en = { title: "Production readiness", description: "One fail-closed view of rights, approval, source health, schedules, Argus, queue history and the bounded canary gate.", rights: "Storage & analysis rights", rightsDetail: "Enabled public sources with both required rights", approval: "Internal & legal approval", approvalDetail: "Enabled sources approved and legally allowed", health: "Operational health", healthDetail: "Enabled sources currently healthy", schedules: "Enabled schedules", schedulerOn: "Scheduler runtime is on", schedulerOff: "Scheduler runtime is off", ready: "READY", blocked: "BLOCKED", queue: "Failed queue history", queueDetail: "{retry} retry eligible · {external} externally blocked", canary: "Canary gate", canaryDetail: "Ready only when every prerequisite is green", blockers: "Release blockers", blockersDetail: "Every item here must be resolved before running a production canary.", noBlockers: "No current blocker", noBlockersDetail: "The bounded canary may be run with its explicit confirmation token.", boundary: "Canary safety boundary", boundaryDetail: "The runner stops on the first unsafe result and never enables schedules.", mode: "Mode", passes: "Passes per source", stop: "Stop conditions", stopDetail: "governance, schedule, parser, repeat growth, remote evidence", rollback: "Rollback", rollbackDetail: "Disable schedules and cancel pending collection work", approvalBlocked: "approval or legal status incomplete", healthBlocked: "source is not healthy", rightsBlocked: "storage or derived-analysis right missing", schedulerRuntimeBlocked: "Scheduler runtime must remain disabled", schedulesBlocked: "{count} schedule(s) are enabled", argusBlocked: "Argus is not healthy and ready" };
const zh: typeof en = { title: "生产就绪", description: "集中查看数据权利、审批、来源健康、计划任务、Argus、失败队列和有界 canary 门禁。", rights: "存储与分析权利", rightsDetail: "已启用公开来源中同时具备两项权利的数量", approval: "内部与法律审批", approvalDetail: "已启用且内部批准、法律允许的来源", health: "运行健康", healthDetail: "当前健康的已启用来源", schedules: "已启用计划", schedulerOn: "定时器运行时已开启", schedulerOff: "定时器运行时已关闭", ready: "就绪", blocked: "阻塞", queue: "失败队列历史", queueDetail: "可重试 {retry} · 外部阻塞 {external}", canary: "Canary 门禁", canaryDetail: "仅所有前置条件通过时才就绪", blockers: "发布阻塞项", blockersDetail: "执行生产 canary 前必须解决这里的所有项目。", noBlockers: "当前无阻塞项", noBlockersDetail: "可使用明确确认口令执行有界 canary。", boundary: "Canary 安全边界", boundaryDetail: "执行器遇到第一个不安全结果即停止，且不会启用计划任务。", mode: "模式", passes: "每来源轮数", stop: "停止条件", stopDetail: "治理、计划、解析、重复增长、远端证据", rollback: "回滚", rollbackDetail: "关闭计划并取消待执行采集任务", approvalBlocked: "审批或法律状态不完整", healthBlocked: "来源不健康", rightsBlocked: "缺少存储或衍生分析权利", schedulerRuntimeBlocked: "定时器运行时必须关闭", schedulesBlocked: "有 {count} 个计划已启用", argusBlocked: "Argus 未达到健康且就绪状态" };
