...
export default async function AdminOverviewPage() {
  ...
  const [businessOpen, collectionOpen, activeChecks, sources, recentRuns, failedRuns, activeJobs, upcomingEvents, activeSignals] = await Promise.all([
    ...
    <section className="admin-page admin-overview-page">
      ...
      <div className="overview-workspace-grid">
        ...
        <aside className="overview-section overview-source-health">
          ...
          {sourceHealthItems.length ? <ul>{sourceHealthItems.map((source) => <li key={source.key}><Link href={`/admin/data-sources/${source.key}`}><span><strong>{source.name}</strong><small>{source.lastSuccessAt ? `${text.lastSuccess} ${date(source.lastSuccessAt, locale)}` : text.neverSucceeded}</small></span><span><StatusPill value={source.enabled ? source.operationalStatus : "PAUSED"} locale={locale} /><small>{Math.round(source.errorRate * 100)}%</small></span></Link></li>)}</ul> : <p className="empty-inline">{text.noSources}</p>}
        </aside>
      </div>
    </section>
  );
