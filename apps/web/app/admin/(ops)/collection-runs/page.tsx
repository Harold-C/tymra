...
export default async function CollectionRunsPage({ searchParams }: { searchParams: { status?: string; incident?: string; source?: string; q?: string; page?: string } }) {
  ...
  const [total, runs, openIncidentCount, running, failedDay] = await Promise.all([
    ...
    prisma.collectionRun.findMany({ where, orderBy: { createdAt: "desc" }, skip: (page - 1) * pageSize, take: pageSize, include: { dataSource: { select: { name: true, key: true } }, incident: true, job: { select: { id: true, status: true, attemptCount: true, maxAttempts: true } } } }),
  ...
  const rows = runs.map((run) => ({
    ...
    cells: {
      run: <><span>{run.id}</span>{run.incident ? <small>{run.incident.category}</small> : null}</>,
  ...
  return <section className="admin-page collection-runs-page">
    ...
    <div className="data-explorer-result-meta"><strong>{text.results}</strong><span>{text.resultCount.replace("{count}", total.toLocaleString())}</span></div>
    ...
    <nav className="data-explorer-pagination" aria-label={text.pagination}><Link aria-disabled={page <= 1} href={href(searchParams, page - 1)}>{text.previous}</Link><span>{text.page.replace("{page}", String(page)).replace("{total}", String(totalPages))}</span><Link aria-disabled={page >= totalPages} href={href(searchParams, page + 1)}>{text.next}</Link></nav>
  </section>;
}

function href(params: Record<string, string | undefined>, page: number) { const query = new URLSearchParams(); for (const [key, value] of Object.entries(params)) if (value && key !== "page") query.set(key, value); query.set("page", String(Math.max(1, page))); return `/admin/collection-runs?${query}`; }
