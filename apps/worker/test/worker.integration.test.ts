...
describe("Worker baseline pipeline", () => {
  ...
  it("persists bounded Eventfinda discovery and details through direct HTTP", async () => {
    ...
    const eventStartsAt = new Date(Date.now() + 30 * 86_400_000);
    const eventEndsAt = new Date(eventStartsAt.getTime() + 2 * 3_600_000);
    const eventYear = eventStartsAt.getUTCFullYear();
    const sourceUrl = `https://www.eventfinda.co.nz/${eventYear}/acceptance-${suffix}/christchurch`;
    const unseenUrl = `https://www.eventfinda.co.nz/${eventYear}/unseen-${suffix}/wellington`;
    ...
    const argusServer = createArgusServer((capture) => {
      ...
      if (capture.workflow_id === "collect_listing") {
        return argusSuccess(capture, { data_schema: "eventfinda-public.collect_listing", schema_version: "1.0.0", extractor: "eventfinda", kind: "listing", title: "Events", canonicalUrl: capture.url, currentPage: 1, totalPages: listingTotalPages, nextUrl: listingTotalPages > 1 ? `${capture.url}/page/2` : null, events: [{ eventId, title: "Acceptance Theatre", sourceUrl, startsAt: eventStartsAt.toISOString(), venueName: "Acceptance Venue", location: "Acceptance Venue, Christchurch", category: "Theatre", imageUrl: null, sponsored: false, ticketAction: "Buy Tickets" }] }, "Events", "a");
      }
      return argusSuccess(capture, { data_schema: "eventfinda-public.collect_detail", schema_version: "1.0.0", extractor: "eventfinda", kind: "event_detail", eventId, title: "Acceptance Theatre", canonicalUrl: sourceUrl, category: "Theatre", description: "Bounded Argus persistence acceptance.", imageUrls: [], venue: { name: "Acceptance Venue", address: { streetAddress: "1 Acceptance Street", addressLocality: "Christchurch", addressRegion: "Canterbury", postalCode: "8011", addressCountry: "New Zealand" }, latitude: -43.53, longitude: 172.63 }, offers: [{ name: "Adult", price: "20.00", priceCurrency: "NZD", availability: "InStock", url: `${sourceUrl}/tickets` }], performers: [], restrictions: null, phoneSales: null, websites: [], listedBy: [], tour: [], occurrences: [{ name: "Acceptance Theatre", description: null, sourceUrl, startDate: eventStartsAt.toISOString(), endDate: eventEndsAt.toISOString(), previousStartDate: null, eventStatus: "EventScheduled", attendanceMode: "OfflineEventAttendanceMode", imageUrls: [], location: null, offers: [], performers: [], organizer: null }] }, "Acceptance Theatre", "a");
    });
    await new Promise<void>((resolve) => argusServer.listen(0, "127.0.0.1", resolve));
    const address = argusServer.address();
