...
export default async function AccommodationsPage({ searchParams }: { searchParams: { view?: string; q?: string; city?: string } }) {
  ...
  const propertyWhere: Prisma.PropertyWhereInput = { isDemo: false, ...(city ? { city } : {}), ...(q ? { OR: [{ canonicalName: { contains: q, mode: "insensitive" } }, { address: { contains: q, mode: "insensitive" } }, { region: { contains: q, mode: "insensitive" } }] } : {}) };
  const rows = view === "properties"
    ? (await prisma.property.findMany({ where: propertyWhere, orderBy: { canonicalName: "asc" }, take: 100, include: { _count: { select: { units: true, listings: true, priceChecks: true } } } })).map((item) => ({ id: item.id, href: `/admin/accommodations/${item.id}`, cells: { record: <><strong>{item.canonicalName}</strong><small>{item.address}</small></>, location: [item.city, item.region].filter(Boolean).join(" · "), type: item.accommodationType, status: <StatusPill value={item.supportStatus} locale={locale} />, units: item._count.units, listings: item._count.listings, checks: item._count.priceChecks } }))
    : (await prisma.sellableUnit.findMany({ where: { isDemo: false, property: propertyWhere, ...(q ? { OR: [{ officialName: { contains: q, mode: "insensitive" } }, { canonicalName: { contains: q, mode: "insensitive" } }] } : {}) }, orderBy: { officialName: "asc" }, take: 100, include: { property: { select: { id: true, canonicalName: true, city: true } }, _count: { select: { listings: true, priceChecks: true, targetRelationships: true } } } })).map((item) => ({ id: item.id, href: `/admin/accommodations/${item.property.id}#unit-${item.id}`, cells: { record: <><strong>{item.officialName}</strong><small>{item.property.canonicalName}</small></>, location: item.property.city, type: item.unitType, status: <StatusPill value={item.status} locale={locale} />, units: item.capacity, listings: item._count.listings, checks: `${item._count.priceChecks} / ${item._count.targetRelationships}` } }));
