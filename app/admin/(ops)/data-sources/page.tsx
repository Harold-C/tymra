...
export default async function DataSourcesPage() {
  ...
  const production = items.filter((item) => item.environments.includes("PRODUCTION") && item.internalApprovalStatus === "APPROVED" && item.legalRightsStatus === "ALLOWED").length;
  const rows = items.map((item) => {
    ...
    return {
      ...
      cells: {
        ...
        governance: <><StatusPill value={item.internalApprovalStatus} locale={locale} /> <StatusPill value={item.legalRightsStatus} locale={locale} /></>,
  ...
  return <section className="admin-page data-sources-page">
    ...
    <AdminTable columns={[{ key: "source", label: text.source }, { key: "type", label: text.type }, { key: "state", label: text.state }, { key: "governance", label: text.governance }, { key: "records", label: text.records }, { key: "run", label: text.lastRun }]} rows={rows} emptyTitle={text.empty} emptyBody={text.emptyBody} />
  </section>;
}
...
const en = { title: "Data sources", description: "Operational health, governance and collection coverage for every non-OTA source.", manualImport: "Manual import", total: "Sources", enabled: "Enabled", healthy: "Healthy", production: "Production ready", catalog: "Source catalog", catalogBody: "Open a source to inspect its runs, schedules, crawl frontier and data.", source: "Data source", type: "Type", state: "Collection state", governance: "Governance", records: "Records / targets", lastRun: "Latest run", empty: "No data sources", emptyBody: "No non-OTA sources are configured." };
const zh: typeof en = { title: "数据来源", description: "查看所有非 OTA 来源的运行健康、治理状态和采集覆盖。", manualImport: "手工导入", total: "来源数量", enabled: "已启用", healthy: "健康", production: "生产就绪", catalog: "来源目录", catalogBody: "打开来源可查看运行、计划、抓取前沿和数据。", source: "数据来源", type: "类型", state: "采集状态", governance: "治理状态", records: "记录 / 目标", lastRun: "最近运行", empty: "暂无数据来源", emptyBody: "尚未配置非 OTA 数据来源。" };
