...
describe("persistent job queue", () => {
  ...
  it("enqueues idempotently and gives one worker an exclusive lease", async () => {
    ...
    const first = await enqueueJob({ type: "SOURCE_HEALTH_CHECK", payload: { source: "demo" }, idempotencyKey: key, queueName });
    const duplicate = await enqueueJob({ type: "SOURCE_HEALTH_CHECK", payload: { source: "demo" }, idempotencyKey: key, queueName });
    expect(duplicate.id).toBe(first.id);
    expect(await prisma.job.count({ where: { idempotencyKey: key } })).toBe(1);

    const claimed = await claimNextJob("worker-a", 30, new Date(), [queueName]);
    expect(claimed?.id).toBe(first.id);
    expect(claimed?.attemptCount).toBe(1);
    expect(await claimNextJob("worker-b", 30, new Date(), [queueName])).toBeNull();
  ...
  it("recovers a job abandoned by a crashed worker and applies the documented retry schedule", async () => {
    ...
    const queued = await enqueueJob({ type: "RATE_COLLECTION", payload: {}, idempotencyKey: key, runAt: oldNow, queueName });
    const claimed = await claimNextJob("worker-a", 10, oldNow, [queueName]);
  ...
  it("moves an exhausted retryable job to the dead-letter state", async () => {
    ...
    const queued = await enqueueJob({ type: "SOURCE_HEALTH_CHECK", payload: {}, idempotencyKey: key, queueName, maxAttempts: 1 });
    const claimed = await claimNextJob("worker-dead-letter", 30, new Date(), [queueName]);
    expect(claimed?.id).toBe(queued.id);
